...
const CategoryList = () => {
    ...
    const handleEdit=(id:string)=>{
        setPopUp(true)
        const res = categories.find((cate:any)=>cate.id === id)
        // console.log(res)
        if(res){
            // const {name} = res
            // setEditField(name)
                ...
                <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td className="py-4 px-6 text-sm font-medium text-right whitespace-nowrap flex justify-center">
                        <button
                            ...
                            // href={category.id}
                            className="text-blue-600 dark:text-blue-500 hover:underline border border-white p-2 hover:border-orange-300 "
                        >
                            Edit
                        </button>
                        {popUp && <Edit_Modal editField={editField}  title="Edit Category" setPopUp={setPopUp} />}
                    </td>
                </tr>
            )
        })
