...
import { games } from "@/app/source/db"
...
const Games = () => {
    ...
    const soft_view = (data: any) => {
        let array: any = []
        data.filter((items: Record<string, unknown>) => items.cate_id === 17).map((item: any, index: number) => {
            array.push(
    ...
    return (
        <div className="p-10 grid lg:grid-cols-3 gap-5">
            <div className="pt-5 divide-y-2 lg:col-span-2">
                <div>
                    <h1 className="py-5 text-4xl font-bold text-gray-700">Games</h1>
                </div>
