...
const AppPage = () => {
    ...
    const [application, setGadget] = useState([])
    const { data, loading, error } = useQuery(get_application)

    useEffect(() => {
        if (data) {
            setGadget(data.posts)
        }
    }, [data, loading])

    const application_view = (data: any) => {
        let array: any = []
        data.filter((items: Record<string, unknown>) => items.cate_id === 16).map((item: any, index: number) => {
            array.push(
    ...
    return (
        <div className="p-10 grid lg:grid-cols-3 gap-5">
            <div className="pt-5 divide-y-2 lg:col-span-2">
                <div>
                    <h1 className="py-5 text-4xl font-bold text-gray-700">Apps</h1>
                </div>
